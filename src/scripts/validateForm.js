  ...
  /*  if(name.value.length == 0){
    ...
    if(!regex.test(name.value) || name.value.length == 0){
        if(!name.classList.contains('nameInputError')){
          ...
          //  let newError = createError('nameTextError', 'Your name shoulds contain minimum 3 characters.')
            let newError = createError('nameTextError', 'Twoje imie musi składać się z conajmniej 3 znaków.')
       ...
       /* if(message.value.length == 0){
        ...
        if(!regex.test(message.value) || message.value.length == 0){
            if(!message.classList.contains('messageInputError')){
                ...
                let newError = createError('messageTextError', "Twoja wiadomość powinna się składać z conajmniej 3 znaków")
